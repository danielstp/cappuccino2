...
class Command(BaseCommand):
        ...
        for carrera in código:
            ...
            try:
                ...
                fecha = timezone.make_aware(
                    datetime.datetime.strptime(código[carrera]["fecha"], self.patronFecha),
    ...
    def descargar(self, código, carrera):
        ...
        meta = u.info()
        fecha = timezone.make_aware(
            datetime.datetime.strptime(str(meta["Last-Modified"]), self.patronFechaApache),
            timezone=timezone.get_current_timezone(),
        )
        if objCarrera.fechaPDF < fecha:
            objCarrera.fechaPDF = fecha
            self.stdout.write(
                self.style.WARNING("PDFNuevo " + objCarrera.fechaPDF.strftime("%Y-%m-%d_%H:%M:%S %z %Z"))
            )
            ...
            with open(
                ...
                + código
                + "/"
                + objCarrera.fechaPDF.strftime("%Y-%m-%d_%H:%M:%S")
                + ".pdf",
                "wb",
            ) as f:
                c = pycurl.Curl()
    ...
    def generarTextos(self):
        ...
        for carrera in carreras:
            ...
            subprocess.run(
                ["pdftotext", "-layout", "-nopgbrk", rutaBase + archivo + ".pdf"], stdout=subprocess.PIPE
            )
            subprocess.run(["sed", "-i", "s/¥/Ñ/g", rutaBase + archivo + ".txt"], stdout=subprocess.PIPE)
    ...
    def cosechar(self, base, objCarrera):
        expBase = (
            r"(\d{5,}) ([A-ZÑ. ]+) (\d{1,2})\s+([VISALUMIJ]{2}) (\d{3,4})-(\d{3,4})\((\d{3}[A-Z]?)\)\s+([A-ZÑ. ]+)"
        )
